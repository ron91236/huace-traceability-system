...
# 凭据不再硬编码：ADMIN_PASS / DB_PASSWORD 环境变量缺失时相关步骤降级
...
if [ -n "$ADMIN_PASS" ]; then
  ADMIN_RESP=$(curl -s -X POST http://127.0.0.1:8080/api/auth/login \
    -H 'Content-Type: application/json' \
    -d "{\"username\":\"admin\",\"password\":\"$ADMIN_PASS\",\"loginType\":\"admin\"}")
...
mysql -uroot -p"$DB_PASSWORD" trace_system -e "SELECT id,order_no,status,submit_time FROM t_order WHERE enterprise_id=11;" 2>/dev/null

echo ""
echo "=== Step 3: Create new order with certId and address ==="
# Get cert for enterprise 11
CERT_ID=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT id FROM enterprise_cert WHERE enterprise_id=11 LIMIT 1;" 2>/dev/null)
ADDR_ID=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT id FROM address WHERE enterprise_id=11 ORDER BY id DESC LIMIT 1;" 2>/dev/null)
GOODS_ID=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT id FROM goods WHERE enterprise_id=11 ORDER BY id DESC LIMIT 1;" 2>/dev/null)
BATCH_ID=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT id FROM batch WHERE enterprise_id=11 ORDER BY id DESC LIMIT 1;" 2>/dev/null)
...
NEW_OID=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT id FROM t_order WHERE enterprise_id=11 ORDER BY id DESC LIMIT 1;" 2>/dev/null)
...
if [ -n "$NEW_OID" ]; then
  ...
  STATUS=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT status FROM t_order WHERE id=$NEW_OID;" 2>/dev/null)
...
if [ "$STATUS" = "PENDING" ] && [ -n "$NEW_OID" ]; then
  ...
  STATUS2=$(mysql -uroot -p"$DB_PASSWORD" trace_system -N -e "SELECT status FROM t_order WHERE id=$NEW_OID;" 2>/dev/null)
